...
function printOutput(num){
    if(num == ''){
        ...
        document.getElementById('current-operand').innerText = getFormatedNum(num);
    }
}
...
function reverseNumForamt(num){
    debugger;
    return Number(num.replace(/,/g,''));
    // return num.toString(num);
}
// alert(reverseNumForamt(getOutput()));
...
for(var i =0;i<number.length;i++){
	number[i].addEventListener('click',function(){
		let output = reverseNumForamt(getOutput);
